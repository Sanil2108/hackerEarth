...
class UndirectedGraph{
    ...
    public void BFS(){
        ...
        Queue q=new Queue();
        q.enqueue(root);
        int nodesInNextLevel=root.n_children;
        visited[root.m_id]=true;
        while(!q.isEmpty()){
            Node tempNode=q.dequeue();
            if(nodesInNextLevel==0){
                nodesInNextLevel=q.R-q.F-1;
                System.out.println(nodesInNextLevel);
            }
            nodesInNextLevel--;
    ...
    public void modifiedBFS(int level){
        boolean[] visited=new boolean[numberOfNodes];
        Queue q=new Queue();
        q.enqueue(root);
        int nodesInNextLevel=root.n_children;
    ...
    public static void problem(){
        ...
        for(int i=0;i<T-1;i++){
            int a=in.nextInt();
            int b=in.nextInt();
            graph.addEdge(nodes[a-1], nodes[b-1]);
        }
        int level=in.nextInt();
        graph.modifiedBFS(level);
    }
}
